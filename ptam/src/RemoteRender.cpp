...
GLuint loadBMP_custom(const char * imagepath){
   ...
   cv::Mat tempImg = cv::imread(imagepath);
   cv::cvtColor(tempImg, tempImg, COLOR_BGR2RGB);
   cv::flip(tempImg, tempImg, 0);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, tempImg.cols, tempImg.rows, 0, GL_RGB, GL_UNSIGNED_BYTE, tempImg.ptr());
...
int main(int argc, char* argv[])
{
   ...
   //logo_faces = LoadOBJ_for_KETILOGO("/home/mobius_keti/Ptam/catkin_ws/devel/lib/opengl_ros/KETI_LOGO.obj", 
   logo_faces = LoadOBJ_for_KETILOGO("/home/keti/Work/catkin_ws/src/ptam_sw/ptam/src/jeep.obj", 
                                      logo_vert_x, logo_vert_y, logo_vert_z, 
                                      logo_uv_u, logo_uv_v, 
                                      logo_norm_x, logo_norm_y, logo_norm_z
                                    );

   // spell_faces = LoadOBJ_for_KETILOGO( "/home/mobius_keti/Ptam/catkin_ws/devel/lib/opengl_ros/KETI_SPELL.obj", 
   //                                     spell_vert_x, spell_vert_y, spell_vert_z, 
   //                                     spell_uv_u, spell_uv_v, 
   //                                     spell_norm_x, spell_norm_y, spell_norm_z
   //                                   );
                                      
   spell_faces = LoadOBJ_for_KETILOGO( "/home/keti/Work/catkin_ws/src/ptam_sw/ptam/src/jeep.obj", 
                                       spell_vert_x, spell_vert_y, spell_vert_z, 
                                       spell_uv_u, spell_uv_v, 
                                       spell_norm_x, spell_norm_y, spell_norm_z
                                     );
   ...
   jeepTexture = loadBMP_custom("/home/keti/Work/catkin_ws/src/ptam_sw/ptam/src/jeep_army.bmp");
   ...
   while(ros::ok())
   {
      ...
      cv::setOpenGlDrawCallback("OpenGL_ROS", on_opengl_modify, &backgroundTex);
...
int LoadOBJ_for_KETILOGO(const char* path, float*& vert_x, float*& vert_y, float*& vert_z, float*& uv_u, float*& uv_v, float*& norm_x, float*& norm_y, float*& norm_z)
{
   float _temp_vert_x[10000];
   float _temp_vert_y[10000];
   float _temp_vert_z[10000];
   unsigned int vert_cnt = 0;

   float _temp_uv_u[10000];
   float _temp_uv_v[10000];
   unsigned int uv_cnt = 0;

   float _temp_norm_x[10000];
   float _temp_norm_y[10000];
   float _temp_norm_z[10000];
   ...
   cout << vert_cnt << " " << uv_cnt << " " << norm_cnt << " " << face_cnt << endl;

   for (int i = 0; i < face_cnt; i++)
   {
      // cout << "f " << _temp_face_vert[i * 3 + 0] << "/" << _temp_face_uv[i * 3 + 0] << "/" << _temp_face_norm[i * 3 + 0];
      // cout << " " << _temp_face_vert[i * 3 + 1] << "/" << _temp_face_uv[i * 3 + 1] << "/" << _temp_face_norm[i * 3 + 1];
      // cout << " " << _temp_face_vert[i * 3 + 2] << "/" << _temp_face_uv[i * 3 + 2] << "/" << _temp_face_norm[i * 3 + 2] << endl;
      ...
      uv_v[i * 3 + 0] = _temp_uv_v[_temp_face_uv[i * 3 + 0] - 1];
      uv_v[i * 3 + 1] = _temp_uv_v[_temp_face_uv[i * 3 + 1] - 1];
      uv_v[i * 3 + 2] = _temp_uv_v[_temp_face_uv[i * 3 + 2] - 1];
...
void on_opengl_modify(void* param)
{
   ...
   */
   ...
   { //Third Object ==> targetObject KETI
      if (mapQuality == 1 && drawObject == true)
      {
         glMatrixMode(GL_PROJECTION);
         glLoadIdentity();
         gluPerspective(45.0f, 640.0f / 480.0f, 0.1f, 100.0f);
         ...
         glRotatef( rotx, 1, 0, 0); // added for object rotation
         ...
         for (int face_i = 0; face_i < spell_faces; face_i++)
         {
            ...
            glTexCoord2f( spell_uv_u[face_i * 3 + 0], spell_uv_v[face_i * 3 + 0]); glVertex3fv(Triangle_Spell[0]);
            glTexCoord2f( spell_uv_u[face_i * 3 + 1], spell_uv_v[face_i * 3 + 1]); glVertex3fv(Triangle_Spell[1]);
            glTexCoord2f(spell_uv_u[face_i * 3 + 2], spell_uv_v[face_i * 3 + 2]); glVertex3fv(Triangle_Spell[2]); 
         }
         glEnd();
